...
@ray.remote
def skeleton_parallel(curr_partition,
    max_partition,
    depth,
    origin_size,
    cg: CausalGraph,
    ...
    return skeleton_partition(curr_partition, max_partition, depth, origin_size, cg, node_ids, alpha, indep_test, stable, background_knowledge, verbose, show_progress, node_names)

@ray.remote
def skeleton_union(cg1: CausalGraph,
                  cg2: CausalGraph):
    return [cg1,cg2]
...
def skeleton_partition(#rough_skeleton,
    ...
    #ray multiprocessing
    '''
    cA = skeleton_parallel.remote(curr_partition, max_partition, depth, origin_size, ray.put(cgA), ray.put(nodes_A), alpha, ray.put(indep_test), stable, background_knowledge, verbose, show_progress, ray.put(node_nameA))
    cB = skeleton_parallel.remote(curr_partition, max_partition, depth, origin_size, ray.put(cgB), ray.put(nodes_B), alpha, ray.put(indep_test), stable, background_knowledge, verbose, show_progress, ray.put(node_nameB))
    result = skeleton_union.remote(cA, cB)
    [cgA, cgB] = ray.get(result)
    '''
    #use mp library
    st = time.time()
    print("multiprocessing start", mp.cpu_count() )
    ...
    print("multiprocessing end:", time.time()-st)
    cg.ts = time.time()-st
    #cgA = skeleton_partition(curr_partition, max_partition, depth, origin_size, cgA, nodes_A, alpha, indep_test, stable, background_knowledge, verbose, show_progress, node_nameA)
    #cgB = skeleton_partition(curr_partition, max_partition, depth, origin_size, cgB, nodes_B, alpha, indep_test, stable, background_knowledge, verbose, show_progress, node_nameB)
    #print("after",len(cgA.G.nodes),len(cgB.G.nodes))
    #if cgA.ts != 0 and cgB.ts!= 0:
        #cg.ts = (cgA.ts+cgB.ts)/2
    #else:
        #cg.ts = max(cgA.ts,cgB.ts)
    ...
    #partitions += (cgA.partitions + cgB.partitions)
...
def skeleton_process(
    ...
    cg2 = skeleton_partition(rough_skeleton, curr_partition, max_partition, cg,node_ids,data, alpha, indep_test,stable,background_knowledge, verbose,show_progress, node_names)
    print(process_id, "complete")
    q.put((process_id, cg2))
    return
...
def skeleton_conditional(
    ...
    '''
    print("received1: ", len(node_ids))
    cg_cp = CausalGraph(len(node_ids), node_ids)
    cg_cp.set_ind_test(indep_test)
    cg_cp.sepset = cg.sepset.copy()
    for x in idx_of_var:
        for y in idx_of_var:
            if (x != y) and cg.G.get_edge(cg.G.nodes[x], cg.G.nodes[y]) is None:
                edge1 = cg_cp.G.get_edge(cg_cp.G.nodes[x], cg_cp.G.nodes[y])
                if edge1 is not None:
                    cg_cp.G.remove_edge(edge1)
    cg_cp.ts = 0
    cg = cg_cp
    '''
    cg = deepcopy(cg)
